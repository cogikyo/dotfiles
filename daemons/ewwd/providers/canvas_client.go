...
// canvas_client.go fetches Spotify Canvas videos (looping MP4s) for tracks.
//
// Auth flow: sp_dc cookie (auto-read from Firefox) + TOTP → access token.
// The sp_dc cookie lasts ~1 year. If canvas stops working, log into
// open.spotify.com in Firefox to refresh it.
//
// Canvas flow: POST protobuf to spclient.wg.spotify.com → CDN URL → download MP4.
...
// spotifyTOTP generates the TOTP code Spotify's web player uses for token requests.
...
// resolveSpDc reads the sp_dc cookie from Firefox's cookie database.
...
// FetchCanvasURL returns the CDN URL for a track's canvas video, or "" if none exists.
