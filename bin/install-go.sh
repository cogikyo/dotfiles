#!/usr/bin/env bash
# bin/install-go.sh - Build and install all Go binaries from dotfiles
#
# Usage:
#   install-go.sh           # Build all binaries
#   install-go.sh hyprd     # Build specific binary
#   install-go.sh ewwd      # Build specific binary
#   install-go.sh --list    # List available binaries
...
declare -A BINARIES=(
    ...
    ["newtab"]="daemons/newtab|.|Firefox new tab HTTP server"
)
...
list_binaries() {
    ...
    for name in "${!BINARIES[@]}"; do
        ...
        if path=$(which "$name" 2>/dev/null); then
            status="${GREEN}✓${NC} $path"
        else
            status="${RED}✗${NC} not installed"
        fi
        printf "  %-12s %-40s %b\n" "$name" "$desc" "$status"
    done | sort
}
