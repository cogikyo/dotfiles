...
// - Render directory, git state, model, context, and rate-limit segments.
// - Keep output as a single ANSI-colored Nerd Font line for stdout.
package main

// main.go defines payload parsing, git status extraction, and statusline rendering.
...
// getGitStatus returns status for dir, or nil when dir is not a usable git worktree.
//
// porcelain=v2 keeps parsing stable.
// --no-optional-locks avoids blocking on a concurrent index lock.
...
func main() {
	...
	// ├─ context window bar ─────────────────────────────────────────────────────┤
	// Percent reflects true usage against the model's full window.
	// Color and fill scale against a 250k comfort budget, not the hard cap.
