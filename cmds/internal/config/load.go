...
// load.go loads daemon YAML files and applies each daemon's fallback policy on error.
...
// Load returns the combined configuration for local daemons.
//
...
// LoadEww loads ewwd config from $HOME/dotfiles/cmds/config/ewwd.yaml.
...
// LoadHypr loads hyprd config from $HOME/dotfiles/cmds/config/hyprd.yaml.
//
...
func logConfigError(section string, err error) {
	...
	fmt.Fprintf(os.Stderr, "cmds: %s config error: %v\n", section, err)
}
