...
The editing pipeline turns accepted source media into a reviewed timeline through inspectable stages: media → measurements → proposals → timeline → render.
...
Raw ASR output is never the root: the corrected source transcript is.
The corrected transcript and caption projection follow the editorial ownership chain; this concern chooses tools and emits measurements or proposals.
...
- Filler-word removal derives from transcript rules over time-anchored source tokens; proposed remove-ranges merge and pad before becoming cuts.
...
- Screen-session dead-end compression follows the genome's jump-cut-plus-time-card law.
...
- Caption rendering follows the editorial projection chain, then uses an authored ASS template and libass with style versioned by the genome.
...
| VibeFrame | agent-harness contract: JSON, dry-run, cost caps | very new (checked 2026-07) |
| FireRed OpenStoryline | local MCP, rough-cut skills, semantic media | new (checked 2026-07), model/API heavy |
...
- Where the human review gate lives per stage (cut proposals vs assembled timeline vs preview render), and how many gates stage 0 actually needs.
