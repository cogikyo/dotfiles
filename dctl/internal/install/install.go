...
// install.go defines install command wiring, step implementations, and healthcheck probes.
...
import (
	...
	"dotfiles/dctl/internal/health"
	"dotfiles/dctl/internal/output"
	"dotfiles/dctl/internal/paths"
	"dotfiles/dctl/internal/pkglist"
	"dotfiles/dctl/internal/prompt"
	"dotfiles/dctl/internal/repos"
	"dotfiles/dctl/internal/secrets"
	"dotfiles/dctl/internal/tui"
	"dotfiles/dctl/internal/update"
)
...
var goBinaries = []goBinary{{"hyprd", "daemons", "./hyprd", "", true}, {"ewwd", "daemons", "./ewwd", "", false}, {"statusline", filepath.Join("config", "claude", "statusline"), ".", filepath.Join("config", "claude", "statusline"), false}, {"newtab", filepath.Join("daemons", "newtab"), ".", "", true}}
...
func installGoServices(ctx context.Context, root paths.Root, out *output.Printer, opts Options) error {
	...
	for _, b := range goBinaries {
		...
		if _, err := os.Stat(src); err != nil {
			src = filepath.Join(root.Dotfiles, b.moduleDir, b.name, b.name+".service")
		}
		if _, err := os.Stat(src); err != nil {
