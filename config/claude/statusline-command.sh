...
if [ "$usage_response" != "" ]; then
    ...
    if [ "$five_hr" != "" ]; then
        ...
        if [ "$five_hr_reset" != "" ]; then
            ...
            [ "$hr_reset_fmt" != "" ] && hr_reset_fmt=" (${hr_reset_fmt})"
        fi
    ...
    if [ "$seven_day" != "" ]; then
        ...
        day_reset_fmt=""
        if [ "$seven_day_reset" != "" ]; then
            day_reset_fmt=$(date -j -f "%Y-%m-%dT%H:%M:%S%z" "${seven_day_reset%%.*}+0000" "+%-m/%-d" 2>/dev/null)
            [ "$day_reset_fmt" != "" ] && day_reset_fmt=" (${day_reset_fmt})"
        fi
        [ "$usage_info" != "" ] && usage_info+="${blu_2} ╼╾ ${N}"
