...
import { sessionContextUsage, sessionMeta, shortDir } from '../shared/session.ts'
...
function StatusLeft(props: { api: TuiPluginApi; sessionID: string }) {
  ...
  return (
    <box flexDirection="row" gap={0}>
      <text fg={colors(props.api.theme.current).brightBlue} wrapMode="none">
        {shortDir(meta().cwd)}
      </text>
      <GitSegment status={git() ?? fallbackGitStatus(props.api)} api={props.api} />
      <ContextSegment api={props.api} sessionID={props.sessionID} revision={revision()} />
    </box>
  )
}
...
function GitCount(props: { value: number; icon: string; fg: ReturnType<typeof colors>[keyof ReturnType<typeof colors>] }) {
  return (
...
function ContextSegment(props: { api: TuiPluginApi; sessionID: string; revision: number }) {
  const usage = () => {
    props.revision
    return sessionContextUsage(props.api, props.sessionID)
  }
  const percent = () => Math.trunc(usage().percent)
  return (
    <Show when={usage().limit && usage().tokens > 0}>
      <box flexDirection="row" gap={0}>
        <Sep api={props.api} />
        <text fg={pressureColor(props.api.theme.current, usage().colorPercent)} wrapMode="none">
          {icons.context}{contextBar(usage().colorPercent)} {percent()}%
        </text>
      </box>
    </Show>
  )
...
async function resolveGitStatus(api: TuiPluginApi, sessionID: string, dir: string): Promise<GitStatus | undefined> {
  ...
  if (hasGitCounters(scopedStatus)) {
    void traceGitStatus({ dir, source: 'opencode-scoped', status: scopedStatus })
    return scopedStatus
  }
  ...
  void traceGitStatus({ dir, source: 'fallback', status, sessionStatus, scopedStatus, openCodeStatus })
  return status
}
