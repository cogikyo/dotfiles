...
import { isExpired, readClaudeCredentials } from "./auth.ts";
import { usageProviders } from "./providers.ts";
import type { UsageProviderSpec } from "./providers.ts";
...
export const anthropicUsage = createUsage(usageProviders.anthropic, claudeAccounts.trend.configDir);
export const anthropicPersonalUsage = createUsage(usageProviders.anthropicPersonal, claudeAccounts.cogikyo.configDir);

function createUsage({ id, label, staleAfterMS }: UsageProviderSpec, configDir: string): ProviderAdapter {
  ...
  async function load(): Promise<ProviderUsage> {
    const access = (await readClaudeCredentials(configDir))[0]?.accessToken;
  ...
  async function tryRecoverAuth(): Promise<string | undefined> {
    ...
    const recovery = (async () => {
      const credentials = await readClaudeCredentials(configDir);
      if (credentials.length === 0) return false;

      const ok = await triggerClaudeRefresh(configDir);
...
async function triggerClaudeRefresh(configDir: string): Promise<boolean> {
  try {
